...
`@passport/core` owns typed contracts, responsive UI, map behavior, filters, IndexedDB, visits/history, progress, and portable JSON. This app owns Washington sample data and copy, region colors, composition, PWA assets, Vite configuration, browser tests, and Pages workflow.
...
`package.json` consumes `file:vendor/passport-core-0.1.0.tgz`. The archive and package lock are versioned inputs: app CI builds without checking out a sibling repository. Local edits in `passport-core` do not affect this app until packed.
...
The app's service worker precaches only built app assets and the bundled sample data. It never caches OSM tile responses in Cache Storage. Browser HTTP caching follows the provider's headers. Offline mode still supports markers, list, filters, details, visits, and backup; a detailed offline basemap is not part of this milestone. Failed tiles produce an explanatory status. Dark mode dims the basemap separately from accessible markers and controls.
...
- Verify the complete official airport participation, six regions, stamp locations, access, and award rules using authoritative sources. Preserve stable IDs. Sample coordinates and region assignments are not verified.
